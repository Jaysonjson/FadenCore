    ...
    public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, AbstractClientPlayerEntity entity, float limbAngle, float limbDistance, float tickDelta, float animationProgress, float headYaw, float headPitch) {
        if(!entity.isInvisible()) {
            ...
            if (data.getRaceSaveData().getRace() != null) {
                ArrayList<RaceCosmetic> cosmetics = data.getRaceSaveData().getRace().getCosmeticPalette().getCosmetics(data.getRaceSaveData().getRaceSub());
                for (String s : data.getRaceSaveData().getCosmetics().getChest()) {
                    RaceCosmetic cosmetic = data.getRaceSaveData().getRace().getCosmeticPalette().getCosmetic(cosmetics, RaceCosmeticSlot.CHEST, s);
