...
public class FadenCoreValueData {
    ...
    public void save() {
        ...
        try {
            FileUtils.writeStringToFile(new File(SaveUtil.getCurrentSaveFull() + "/item_value_data.json"), FadenCore.GSON.toJson(FadenCore.DATA), StandardCharsets.UTF_8);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void load() {
        if(!FadenCore.MODULES.itemValues) return;
        File dataFile = new File(SaveUtil.getCurrentSaveFull() + "/item_value_data.json");
        if(dataFile.exists()) {
            try {
